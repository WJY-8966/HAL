...
from model.MMVFLModel import ModalAttentionModel,MMVFLModel
...
def evaluate_retrieval(model, dataloader, device='cuda:1'):
    model.eval()
    all_img, all_txt = [], []
...
if __name__ == '__main__':
    ...
    aligned_test_loader = DataLoader(aligned_test_set, batch_size=batch_size,
                                                    shuffle=False)
    aligned_val_loader = DataLoader(aligned_val_set, batch_size=batch_size,shuffle=False)

    aligned_model = MMVFLModel().to(device)
    ...
    for epoch in range(num_epochs):
        ...
        aligned_val_metrics = evaluate_retrieval(aligned_model, aligned_val_loader, device)
        print(f"Validation Metrics: {aligned_val_metrics}")
